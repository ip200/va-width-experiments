...
\begin{document}
	...
	\begin{abstract}
		Venn--Abers predictors return two calibrated probabilities, $p_0$ and $p_1$, but the interpretation of the interval width $w=p_1-p_0$ has remained unclear. In this paper we study the width through the local block structure of isotonic regression. We show that it is closely related to the inverse effective size of the local PAV block and derive scaling relationships with calibration sample size, score density, label variability, and the slope of the underlying probability curve. The results suggest that Venn--Abers width is primarily a measure of local calibration support rather than a general measure of predictive uncertainty. Controlled scaling experiments recover the predicted behaviour, while bootstrap and crowd-annotation experiments show a much stronger relationship with calibration instability than with label ambiguity. Real-data experiments on three public tabular benchmarks (UCI Adult, UCI Bank Marketing, and UCI Spambase) support this interpretation: deliberately thinning local calibration support monotonically inflates Venn--Abers width and calibration instability without producing any response in base-model epistemic uncertainty. In quantitative uncertainty decompositions, Venn--Abers width provides substantial, statistically decisive explanatory power for calibration instability beyond outcome ambiguity and base-model uncertainty ($\Delta R^2 > 0$). These results clarify that Venn--Abers width captures a calibration-specific component of epistemic uncertainty associated with finite local support.
	\end{abstract}
...
\section{Introduction}
	...
	Venn--Abers predictors (VAPs) \cite{vovk2014, vovk2015} are conformal calibration algorithms for binary classification that map a scalar scoring rule $s = g(x)$ to a pair of well-calibrated probabilities,
	\begin{equation}
		p_0(s),\qquad p_1(s),
		\label{eq:p0_p1}
	\end{equation}
	obtained by fitting isotonic regression after hypothetically augmenting a calibration sample of size $n$ with the test instance under labels $y=0$ and $y=1$, respectively. From this pair, the calibrated point prediction is defined as the midpoint
	\begin{equation}
		\hat p(s)=\frac{p_0(s)+p_1(s)}{2},
		\label{eq:phat_def}
	\end{equation}
	and the Venn--Abers interval width is
	\begin{equation}
		w_n(s)=p_1(s)-p_0(s).
		\label{eq:wn_def}
	\end{equation}
	We denote the aleatoric Bernoulli outcome variance under the true latent conditional probability $\theta(s) = \mathbb{P}(Y=1 \mid s)$ by
	\begin{equation}
		v(s)=\theta(s)(1-\theta(s)).
		\label{eq:vs_def}
	\end{equation}
	We retain the subscript $n$ when discussing asymptotic dependence on calibration sample size and suppress it in fixed-sample empirical sections.
	...
	In this paper we show that contrary to the common intuition that interval width reflects aleatoric ambiguity in the underlying outcome ($v(s)$), Venn--Abers width is primarily a measure of finite-sample calibration support and local calibration leverage. Using the Greatest Convex Minorant (GCM) representation of isotonic regression, we show that the width is determined by the characteristic active block size $N_{\mathrm{eff}}(s)$ fitted by the Pool Adjacent Violators (PAV) algorithm:
	\begin{equation}
	...
	\begin{equation}
		...
		\label{eq:intro_ucal}
	\end{equation}
	which has the predicted isotonic $n^{-1/3}$ scale. We use this quantity as an index of calibration instability rather than as an exact standard error.

	We test these relationships in a number of experiments. These include monotonic and non-monotonic synthetic examples, bootstrap resampling, a crowd-annotation simulation, and controlled local support interventions on three real-world tabular benchmarks (UCI Adult, UCI Bank Marketing, and UCI Spambase). We also increase the calibration sample size to check whether the finite-sample density, ambiguity, and slope coefficients move towards the values suggested by the scaling argument. The results are broadly consistent with the proposed interpretation. In the real-data experiments, thinning local calibration support increases Venn--Abers width and pointwise calibration instability while leaving base-model epistemic uncertainty completely invariant. In quantitative decompositions across held-out test points, outcome ambiguity explains baseline Bernoulli variability, while Venn--Abers width adds substantial incremental explanatory power (e.g., $\Delta R^2 = +0.0208$ on Adult, $+0.0171$ on Bank, and $+0.0045$ on Spambase; all 95\% bootstrap CIs strictly positive), supporting the interpretation that width captures local calibration support rather than general predictive uncertainty.
...
\section{Problem Setup and Notation}
	...
	The Venn--Abers predictor outputs the interval $[p_0(s), p_1(s)]$. By construction, $p_0(s) \le p_1(s)$. As introduced in Section~\ref{sec:intro}, the calibrated point prediction $\hat{p}(s)$ and the interval width $w_n(s)$ are:
	\begin{equation}
		\hat{p}(s) = \frac{p_0(s) + p_1(s)}{2}, \quad w_n(s) = p_1(s) - p_0(s).
	\end{equation}
	The pair $(p_0(s), p_1(s))$ is part of the multiprobabilistic validity construction of Venn--Abers predictors \cite{vovk2015}. It should not be interpreted as a classical confidence interval for an unknown Bernoulli probability. All of our experiments were carried out using the open-source \texttt{venn-abers} Python package library \cite{vennaberspkg}.
	...
	\begin{itemize}
		\item Inserting $y = 0$ leaves the cumulative sum unchanged for subsequent indices but shifts the index scale.
		\item Inserting $y = 1$ shifts the cumulative sum up by $1$ for all subsequent indices.
	\end{itemize}
	...
	\begin{figure}[ht]
		...
		\begin{tikzpicture}[scale=1.2]
			...
			% Active Block (N=4)
			\draw[blue, thick] (1.5, 0.5) -- (5.5, 2.5) node[pos=0.7, above left] {Base GCM};
			
			% Adding y=1 (pulls slope up)
			\draw[red, thick, dashed] (1.5, 0.5) -- (5.5, 3.0) node[pos=0.3, above left] {Insert $y=1 \implies p_1$};
			
			% Adding y=0 (pulls slope down)
			\draw[green!60!black, thick, dashed] (1.5, 0.5) -- (5.5, 2.0) node[pos=0.8, below right] {Insert $y=0 \implies p_0$};
			
			% Interval Width Bracket
			\draw[<->, thick] (5.7, 2.0) -- (5.7, 3.0) node[midway, right] {Diff $\approx 1$};
			
			% N bracket
			\draw[<->, thick] (1.5, -0.2) -- (5.5, -0.2) node[midway, fill=white] {Local PAV block size $N_{\mathrm{eff}}$};
			\draw[dotted, thick] (1.5, 0.5) -- (1.5, -0.2);
			\draw[dotted, thick] (5.5, 2.0) -- (5.5, -0.2);
		\end{tikzpicture}
		\caption{Conceptual illustration of the GCM logic. The test point falls into a local active block of length $N$. Inserting $y=1$ or $y=0$ shifts the local cumulative sum by $1$, altering the GCM slope. Thus, the probability interval width $p_1 - p_0$ is approximately $1/N$.}
	...
	We interpret the width therefore as a local property of the fitted calibration mapping rather than as one general measure of uncertainty. It depends jointly on calibration size, local score support, Bernoulli variation, and the shape of the calibration curve. Equation~\ref{eq:ucal} adds back the Bernoulli variance when the quantity of interest is probability-scale instability.
	...
	\end{equation}
	The regular case $\alpha=1$ gives the $n^{-2/3}$ rate. An order-two flat or cusp-like regime gives $n^{-4/5}$, while a smooth monotone cubic flat point with first non-zero derivative of order three gives $n^{-6/7}$. This distinction is useful because a twice-differentiable monotone function cannot generally have an interior point with $\theta'(s_0)=0$ and $\theta''(s_0)\ne0$ while remaining monotone on both sides.
	...
	We generate a 1D score $s$ uniformly distributed in $[-1, 1]$. To verify the standard scaling exponents (Equation~\ref{eq:scaling_law_final}), we evaluate the interval width $w(s)$ at a fixed test point $s_{\text{test}} = 0.5$ (chosen in the interior of the score range to avoid boundary effects of the Greatest Convex Minorant) while systematically varying one variable at a time:
	\begin{enumerate}
		\item \textbf{Density $f(s)$:} We vary the local score density while holding calibration size fixed.
		\item \textbf{Ambiguity $\theta(s)(1-\theta(s))$:} We vary the label noise at $s$.
		\item \textbf{Slope $\theta'(s)$:} We vary the derivative of the probability curve.
	\end{enumerate}
	For the non-monotonicity and transformation invariance experiments, we construct specialized functions (e.g., non-monotonic peaks and flat boundary extrema) as described in Section~\ref{subsec:non_monotonic_results}.
	...
	\begin{table}[ht]
		...
		\begin{tabular}{rcccc}
			...
			\midrule
			100 & 0.1564 & 0.1131 & 0.9127 & 0.1236 \\
			250 & 0.1045 & 0.1118 & 0.8603 & 0.1234 \\
			500 & 0.0658 & 0.0990 & 0.7802 & 0.1197 \\
			1000 & 0.0423 & 0.0774 & 0.8520 & 0.1197 \\
			2000 & 0.0261 & 0.0586 & 0.8779 & 0.1161 \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	Table~\ref{tab:calibration_resampling} shows that both quantities decrease as the calibration set becomes larger. The correlation between width and resampling SD is also high for most of the calibration sizes, generally between about 0.85 and 0.91.
	
	We next look at individual test points. Equation~\ref{eq:variance_width_relation} suggests that raw width alone is not on the same scale as the standard deviation of the calibrated probability, since the Bernoulli variance also enters the calculation. In a classifier-based bootstrap experiment at $N = 500$, the index $U_{\mathrm{cal}}=\sqrt{\hat p(1-\hat p)w}$ has Pearson correlation $0.5300$ with bootstrap SD, compared with $0.4287$ for raw width. The corresponding Spearman correlations are $0.4247$ and $0.4072$. In a held-out multiplicative fit, the mean absolute error falls from $0.0627$ for width to $0.0389$ for $U_{\mathrm{cal}}$. The results suggest that $U_{\mathrm{cal}}$ is a better probability-scale summary than width alone in this experiment. We do not use either quantity as an exact pointwise standard error.
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.92\textwidth]{width_vs_bootstrap_instability.png}
		\caption{Pointwise comparison of Venn--Abers interval width and probability-scale calibration instability in the classifier-based bootstrap experiment ($N_{\mathrm{cal}}=500$, $M=200$ resamples). Panel~(a) compares raw width $w$ with bootstrap standard deviation $\sigma_{\text{boot}}$ (Pearson $r = 0.4287$, Spearman $\rho = 0.4072$). Panel~(b) compares the calibration instability index $U_{\text{cal}}=\sqrt{\hat p(1-\hat p)w}$ with bootstrap standard deviation (Pearson $r = 0.5300$, Spearman $\rho = 0.4247$). In a held-out multiplicative fit, the mean absolute error is reduced from $0.0627$ for width to $0.0389$ for $U_{\text{cal}}$.}
		\label{fig:width_vs_bootstrap_instability}
	\end{figure}

	\subsection{Empirical Verification of Scaling Laws}
	
	We first check the sample-size rates in a simple classifier-free experiment. We generate $S\sim U(0,1)$ and $Y\mid S=s\sim\operatorname{Bernoulli}(0.2+0.6s)$, and evaluate a fixed interior score $s_0=0.5$ over increasing calibration sizes. The fitted log--log slope of mean width is $-0.6390$ (95\% CI $[-0.6768,-0.6012]$), compared with the predicted $-2/3$. The bootstrap standard deviation of the calibrated probability has slope $-0.3172$ (95\% CI $[-0.3273,-0.3072]$), close to the isotonic $-1/3$ rate, while $U_{\mathrm{cal}}$ has slope $-0.3285$ (95\% CI $[-0.3426,-0.3144]$), whose interval contains $-1/3$. The bootstrap-SD confidence interval does not contain the exact theoretical exponent, so we interpret this result as close finite-sample agreement rather than exact confirmation.
	
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{idealised_scaling_laws.png}
\caption{Idealised scaling of Venn--Abers interval width and probability-scale calibration instability. Panel (a) shows the mean interval width as a function of calibration size, together with the predicted $n^{-2/3}$ reference rate. Panel (b) shows the bootstrap standard deviation of the calibrated probability and $U_{\mathrm{cal}}=\sqrt{\hat p(1-\hat p)w}$, together with the predicted $n^{-1/3}$ reference rate. The fitted slopes are $-0.6390$ for width, $-0.3172$ for bootstrap standard deviation, and $-0.3285$ for $U_{\mathrm{cal}}$.}
...
\input{table_w2_exponent_progression.tex}
	
	The coefficients are quite far from the theoretical values for the smaller calibration samples, but the gap reduces as the sample becomes larger. At $n=500$, the three local coefficients are approximately $(-0.425,-0.129,+0.390)$ for density, ambiguity, and slope. By $n=8000$ they are approximately $(-0.657,-0.296,+0.590)$, and at $n=16000$ they are $(-0.660,-0.388,+0.690)$. The theoretical targets $(-2/3,-1/3,+2/3)$ lie inside the empirical confidence intervals at the larger calibration sizes. There is some sampling variability at $n=32000$, where the point estimates move back to approximately $(-0.615,-0.303,+0.614)$, but the theoretical values remain inside the corresponding intervals.
	
	The mean absolute discrepancy from the theoretical exponents decreases with $\log n$ at slope $-0.0435$ ($p=0.0123$). The individual absolute-error trends are also negative for density ($-0.0495$, $p=0.0093$), ambiguity ($-0.0336$, $p=0.0343$), and slope ($-0.0475$, $p=0.0162$). Across all calibration sizes, the pooled regression is
	\begin{equation}
		\log w
		=
		-0.6815\log n
		-0.5467\log f
		-0.2446\log[\theta(1-\theta)]
		+0.5303\log|\theta'|
		+\text{constant},
	\end{equation}
...
\end{figure}
	...
	\begin{itemize}
		\item \textbf{Regime A (Standard Monotonic):} $\theta(s) = 0.5 + 0.3 s$ (positive slope $\theta' = 0.3$). The empirical exponent of width vs $N$ is $-0.660$, matching the theoretical exponent of $-2/3 \approx -0.667$.
		\item \textbf{Regime B (Monotonicity-Violating):} $\theta(s) = 0.5 - 0.3 s$ (negative slope $\theta' = -0.3$). Because the probability decreases, the PAV algorithm merges all points into a single block containing a non-vanishing fraction of the $N$ calibration observations. The empirical exponent is $-1.004$, matching the theoretical value of $-1.000$.
		\item \textbf{Regime C (Local Extremum with Violation):} $\theta(s) = 0.5 - 0.4 s^2$ on $s \in [-1, 1]$ (parabolic middle peak, $\theta'(0) = 0$). Here, the right half ($s > 0$) decreases, violating monotonicity and forcing a macroscopic merge. The peak at $s=0.0$ is merged into this violating block, and the empirical exponent is $-1.097$, aligning with the $-1.000$ limit.
		\item \textbf{Regime D (Order-Two Flat/Cusp Regime):} $\theta(s) = 0.5 + 0.4 s^2 \operatorname{sign}(s)$ on $s \in [-1, 1]$. The function is monotone and has local change of order $|s|^2$, but it is not twice differentiable at the origin. Equation~\ref{eq:general_order_scaling} with $\alpha=2$ therefore predicts the $-4/5$ sample-size exponent. The empirical exponent is $-0.802$, close to $-4/5=-0.800$. A smooth monotone cubic flat point would instead correspond to $\alpha=3$ and the $-6/7$ rate.
	\end{itemize}
	...
	\begin{figure}[ht]
		...
		\caption{Log-log plots of Venn--Abers interval width vs. calibration set size for standard monotonic (Regime A), monotonicity-violating (Regime B), middle-peak violation (Regime C), and order-two flat/cusp (Regime D) setups. The empirical exponents are close to the corresponding predicted rates of $-2/3$, $-1$, $-1$, and $-4/5$.}
	...
	\input{table_cifar_correlations.tex}
	...
	\begin{itemize}
		\item The VAP interval width has a positive correlation with \textbf{Bootstrap Instability} (Pearson $\approx 0.69$, Spearman $\approx 0.53$). This suggests that wider intervals tend to occur for test points for which the calibrated probability is more sensitive to the particular finite calibration sample drawn.
		\item Conversely, the VAP interval width has a near-zero or slightly negative correlation with \textbf{Annotator Disagreement} (Pearson $\approx 0.013$, Spearman $\approx -0.008$) and \textbf{Annotator Entropy} (Pearson $\approx 0.016$, Spearman $\approx -0.008$). 
	\end{itemize}
	...
	\begin{figure}[htbp]
		...
		\caption{Controlled local calibration-support thinning on real tabular datasets (UCI Adult, UCI Bank Marketing, UCI Spambase). Panel~(a) shows mean Venn--Abers interval width $\bar{w}$ as local calibration support in the 10\% score rank neighbourhood is thinned from 100\% down to 12.5\%. Panel~(b) shows empirical calibration bootstrap standard deviation $\sigma_{\mathrm{cal}}$. Panel~(c) shows base-model epistemic uncertainty $E_{\mathrm{model}}$, which remains strictly invariant because the base classifier is held fixed while local calibration support is manipulated.}
		\label{fig:real_data_local_support}
	\end{figure}

	The empirical results provide direct support for the theoretical framework:
	\begin{enumerate}
		\item \textbf{Venn--Abers Width Responds Monotonically to Calibration Support:} As the retained fraction of calibration observations in the local neighbourhood decreases from 100\% to 12.5\%, the mean Venn--Abers width increases monotonically across all three datasets (Panel~a). On UCI Adult, mean width rises from $0.0051$ to $0.0278$; on UCI Bank Marketing, from $0.0050$ to $0.0381$; and on UCI Spambase, from $0.0734$ to $0.2079$.
		\item \textbf{Calibration Instability Strongly Corresponds to Width:} As local calibration support is removed, the empirical bootstrap standard deviation of the calibrated probabilities ($\sigma_{\mathrm{cal}}$) exhibits an identical upward progression (Panel~b), rising from $0.0243$ to $0.0442$ on Adult, from $0.0289$ to $0.0558$ on Bank Marketing, and from $0.0671$ to $0.0898$ on Spambase.
		\item \textbf{Strict Decoupling from Base-Model Uncertainty:} Panel~(c) displays the base-model epistemic uncertainty $E_{\mathrm{model}}(x)$. Because the base classifier is kept completely fixed throughout the calibration support intervention, $E_{\mathrm{model}}$ remains perfectly horizontal across all thinning levels ($0.0424$ on Adult, $0.0559$ on Bank, and $0.1169$ on Spambase). This confirms that manipulating calibration support alters Venn--Abers width and calibration instability without confounding by base-model epistemic uncertainty.
	\end{enumerate}

	\paragraph{Reverse Intervention: Manipulating Training Support.}
	In the reverse intervention, we hold the calibration set fixed and subsample the training partition from 100\% down to 25\%. As shown in Figure~\ref{fig:training_support_epistemic}, decreasing training support systematically inflates base-model epistemic uncertainty $E_{\mathrm{model}}$ (Panel~a, increasing from $0.0177$ to $0.0640$ on Adult, and from $0.0250$ to $0.0847$ on Bank). In contrast, Venn--Abers interval width remains virtually constant (Panel~b, staying at approximately $0.0070$ on Adult and $0.0090$ on Bank). This confirms that Venn--Abers width reflects calibration data support rather than base-model training support.

	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.88\textwidth]{training_support_epistemic.png}
		\caption{Reverse intervention altering base-model training support. Panel~(a) shows that reducing training support systematically increases base-model epistemic uncertainty $E_{\mathrm{model}}$. Panel~(b) demonstrates that Venn--Abers width remains essentially stable when calibration support is held fixed.}
	...
	\input{table_real_data_nested.tex}

	Across all three datasets, Model~A (outcome ambiguity alone) accounts for a large baseline fraction of calibration variability ($R^2$ of $0.8823$ on Adult, $0.9149$ on Bank, and $0.9506$ on Spambase). Adding base-model epistemic uncertainty (Model~B) produces modest improvements. Crucially, adding Venn--Abers width $w$ (Model~C) yields a substantial and statistically decisive increase in explanatory power across all benchmarks: $\Delta R^2 = +0.0208$ on Adult (95\% bootstrap CI $[0.0095, 0.0377]$), $\Delta R^2 = +0.0171$ on Bank (95\% bootstrap CI $[0.0116, 0.0282]$), and $\Delta R^2 = +0.0045$ on Spambase (95\% bootstrap CI $[0.0015, 0.0090]$). All 95\% bootstrap confidence intervals for $\Delta R^2$ are strictly positive and bounded away from zero. These findings confirm that Venn--Abers width captures calibration-specific support information that is distinct from both outcome ambiguity and base-model epistemic uncertainty.

	% ============================================================
	\section{Comparison with Related Calibration Methods}
	\label{sec:alternatives}

	For comparison, we also look at the sampling variation of three standard post-hoc calibration methods: Isotonic Regression (IR), Platt Scaling (PS), and Histogram Binning (HB). These methods do not produce a Venn--Abers-style interval, so an equivalent local instability measure has to be estimated by resampling.
...
\section{Discussion and Practical Implications}
	...
	Outcome ambiguity is a separate quantity. The $v^{-1/3}$ term in Proposition~1 is conditional on density and local slope, and these variables can move together in a fitted model. In the crowd-annotation experiment, width exhibits virtually zero correlation with annotator disagreement or entropy. Furthermore, on real tabular benchmarks, outcome ambiguity and calibration support width combine to explain finite-sample calibration instability, with width providing substantial incremental explanatory power ($\Delta R^2 > 0$). Width on its own should therefore not be used as a general indicator of ambiguous labels.

	It is equally essential to separate calibration support from base-model epistemic uncertainty. In our controlled thinning experiments, thinning the local calibration set monotonically increased width and calibration instability, while base-model epistemic uncertainty remained completely unchanged. Conversely, subsampling the training set increased base-model uncertainty without affecting calibration width. Thus:
	\begin{equation*}
...
\section{Conclusion}
	...
	The empirical results also show where this interpretation should stop. Width is not a direct measure of label ambiguity, distribution shift, or base-model parameter uncertainty. Across controlled experiments on real tabular benchmarks, reducing local calibration support increases Venn--Abers width and calibration instability without producing any response in base-model epistemic uncertainty. The results suggest therefore that Venn--Abers width is most useful as a description of local support and sensitivity of the calibration mapping, rather than as a general scalar uncertainty score.
	...
	In this appendix, we show that the heuristic scaling law proposed in Equation~\ref{eq:scaling_law_final} is invariant under any strictly increasing transformation of the scores.
	
	Let $g: \mathbb{R} \to \mathbb{R}$ be a strictly increasing, differentiable function defining a score transformation $s_{\text{new}} = g(s)$. Let $\theta(s)$ and $f(s)$ denote the true probability function and score density under the original score space, respectively. Under the transformed score space:
	\begin{enumerate}
		\item The true probability function becomes $\theta_{\text{new}}(s_{\text{new}}) = \theta(s)$.
		\item By the chain rule, the derivative of the true probability with respect to the new score is:
		\begin{equation}
			\theta'_{\text{new}}(s_{\text{new}}) = \frac{d\theta_{\text{new}}}{d s_{\text{new}}} = \frac{d\theta}{ds} \frac{ds}{ds_{\text{new}}} = \theta'(s) \cdot \frac{1}{g'(s)}
		\end{equation}
		\item The probability density of the scores transforms under change-of-variables as:
		\begin{equation}
			f_{\text{new}}(s_{\text{new}}) = f(s) \cdot \left| \frac{ds}{ds_{\text{new}}} \right| = f(s) \cdot \frac{1}{g'(s)}
		\end{equation}
	\end{enumerate}
	
	Substituting these transformed terms into the heuristic scaling law (Equation~\ref{eq:scaling_law_final}) for the transformed scores:
	\begin{align}
		w(s_{\text{new}}) &\propto f_{\text{new}}(s_{\text{new}})^{-2/3} \cdot (\theta_{\text{new}}(1-\theta_{\text{new}}))^{-1/3} \cdot (\theta'_{\text{new}}(s_{\text{new}}))^{2/3} \\
		&= \left( \frac{f(s)}{g'(s)} \right)^{-2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot \left( \frac{\theta'(s)}{g'(s)} \right)^{2/3} \\
		&= f(s)^{-2/3} \cdot (g'(s))^{2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot (\theta'(s))^{2/3} \cdot (g'(s))^{-2/3}
	\end{align}
	Notice that the derivative factors $(g'(s))^{2/3}$ and $(g'(s))^{-2/3}$ cancel each other exactly:
	\begin{equation}
		w(s_{\text{new}}) \propto f(s)^{-2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot (\theta'(s))^{2/3} = w(s)
	\end{equation}
	This cancellation shows that the scaling law is mathematically invariant to monotonic score transformations (even if the transformation derivative $g'(s)$ varies abruptly or non-linearly), which is consistent with the rank-ordering nature of the PAV algorithm.
	...
	\begin{thebibliography}{20}
		...
		\bibitem{vovk2014}
		V.~Vovk and I.~Petej.
		\newblock Venn--Abers Predictors.
		\newblock In {\em Proceedings of the Thirtieth Conference on Uncertainty in Artificial Intelligence (UAI)}, pages 829--838, 2014.
		
		\bibitem{vovk2015}
		V.~Vovk, I.~Petej, and V.~Fedorova.
		\newblock Large-scale probabilistic predictors with and without guarantees of validity.
		\newblock In {\em Advances in Neural Information Processing Systems (NeurIPS)}, volume~28, pages 892--900, 2015.

		\bibitem{manokhin2017multiclass}
		V.~Manokhin.
		\newblock Multi-class probabilistic classification using inductive and cross Venn-Abers predictors.
		\newblock In {\em Proceedings of the Sixth Workshop on Conformal and Probabilistic Prediction and Applications}, volume~60 of {\em Proceedings of Machine Learning Research}, pages 228--240. PMLR, 2017.

		\bibitem{nouretdinov2018ivapdist}
		I.~Nouretdinov, D.~Volkhonskiy, P.~Lim, P.~Toccaceli, and A.~Gammerman.
		\newblock Inductive Venn-Abers predictive distribution.
		\newblock In {\em Proceedings of the Seventh Workshop on Conformal and Probabilistic Prediction and Applications}, volume~91 of {\em Proceedings of Machine Learning Research}, pages 15--36. PMLR, 2018.

		\bibitem{petej2026regressors}
		I.~Petej and V.~Vovk.
		\newblock Inductive Venn-Abers and related regressors.
		\newblock {\em arXiv preprint arXiv:2605.06646}, 2026.

		\bibitem{groeneboom1985}
		P.~Groeneboom.
		\newblock Estimating a monotone density.
		\newblock In {\em Proceedings of the Berkeley Conference in Honor of Jerzy Neyman and Jack Kiefer}, volume~2, pages 539--555. Wadsworth, 1985.

		\bibitem{wright1981}
		F.~T. Wright.
		\newblock The asymptotic behavior of monotone regression estimates.
		\newblock {\em The Annals of Statistics}, 9(2):443--448, 1981.
		...
		\bibitem{groeneboom2014}
		P.~Groeneboom and G.~Jongbloed.
		\newblock {\em Nonparametric Estimation under Shape Constraints}.
		\newblock Cambridge University Press, 2014.
		
		\bibitem{han2022}
		Q.~Han and K.~Kato.
		\newblock Berry--Esseen bounds for Chernoff-type nonstandard asymptotics in isotonic regression.
		\newblock {\em The Annals of Applied Probability}, 32(2):1459--1498, 2022.

		\bibitem{kumar2019}
		A.~Kumar, P.~S. Liang, and T.~Ma.
		\newblock Verified uncertainty calibration.
		\newblock In {\em Advances in Neural Information Processing Systems (NeurIPS)}, volume~32, pages 4228--4238, 2019.

		\bibitem{vaicenavicius2019}
		J.~Vaicenavicius, D.~Widmann, C.~Andersson, F.~Lindsten, J.~Roll, and T.~Sch{\"o}n.
		\newblock Evaluating model calibration in classification.
		\newblock In {\em International Conference on Artificial Intelligence and Statistics (AISTATS)}, volume~89 of {\em Proceedings of Machine Learning Research}, pages 3459--3467. PMLR, 2019.
		...
		\newblock In {\em Advances in Neural Information Processing Systems (NeurIPS)}, volume~33, pages 3711--3723, 2020.
		
		\bibitem{peck2019}
		J.~Peck, B.~Goossens, and Y.~Saeys.
		\newblock Detecting adversarial examples with inductive Venn-ABERS predictors.
		\newblock In {\em Proceedings of the 27th European Symposium on Artificial Neural Networks, Computational Intelligence and Machine Learning (ESANN)}, pages 251--256, 2019.

		\bibitem{johansson2023}
		U.~Johansson, T.~L{\"o}fstr{\"o}m, and C.~S{\"o}nstr{\"o}d.
		\newblock Well-calibrated probabilistic predictive maintenance using Venn-Abers.
		\newblock {\em arXiv preprint arXiv:2306.06642}, 2023.

		\bibitem{vennaberspkg}
		Venn--Abers Python Library.
		\newblock \url{https://github.com/ip200/venn-abers}
