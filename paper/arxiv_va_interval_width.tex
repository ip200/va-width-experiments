...
\section{Problem Setup and Notation}
	...
	\begin{equation}
		w_n(s) \;\sim\; \underbrace{n^{-2/3}f(s)^{-2/3}}_{\text{calibration support}} \;\times\; \underbrace{v(s)^{-1/3}}_{\text{outcome variability}} \;\times\; \underbrace{[\theta'(s)]^{2/3}}_{\text{local geometry}}.
		\label{eq:scaling_decomp}
	\end{equation}
	Width is thus not one of the conventional predictive uncertainty components: it is an empirical sensitivity measure arising from the calibration step, influenced by score-conditional outcome noise but fundamentally distinct from it. We refer primarily to this quantity as local calibration support or leverage. In the conventional aleatoric--epistemic taxonomy, it can be viewed as a calibration-specific component of epistemic uncertainty arising from finite local calibration data. Equation~\ref{eq:ucal} incorporates the score-conditional outcome variance $v(s)$ back into the expression when the quantity of interest is probability-scale instability.
...
\end{figure}
	...
	\begin{enumerate}
		...
		\item \textbf{Base-Model Uncertainty Control:} Panel~(c) displays the base-model epistemic uncertainty $E_{\mathrm{model}}(x)$. Because the base model and training data are held fixed, estimated base-model epistemic uncertainty is unchanged by construction ($0.0424$ on Adult, $0.0559$ on Bank, and $0.1169$ on Spambase). This isolates the response of width to changes in calibration support, verifying that the empirical inflation in width and calibration instability is not confounded by base-model variation.
	\end{enumerate}

	\paragraph{Reverse Intervention: Manipulating Training Support.}
	In the reverse intervention, we hold the calibration set fixed and subsample the training partition from 100\% down to 25\%. As shown in Figure~\ref{fig:training_support_epistemic}, decreasing training support systematically inflates base-model epistemic uncertainty $E_{\mathrm{model}}$ (Panel~a, increasing from $0.0225$ to $0.0591$ on Adult, from $0.0284$ to $0.0890$ on Bank, and from $0.0000$ to $0.1396$ on Spambase). In contrast, Venn--Abers interval width is considerably less responsive to reduced training support than base-model bootstrap variability, particularly on Adult (Panel~b, shifting marginally from $0.0112$ to $0.0075$) and Bank ($0.0123$ to $0.0121$). Some change in width remains visible on Spambase ($0.0587$ to $0.0429$) because retraining the base predictor changes the score function $g(x)$, altering the positions and ranking of calibration points even though the calibration instances themselves are fixed. These results illustrate that Venn--Abers width is primarily sensitive to calibration data support rather than base-model training set size.
	...
	Across all three datasets, Model~A (score-conditional outcome ambiguity alone) accounts for a large baseline fraction of calibration variability ($R^2$ of $0.8823$ on Adult, $0.9149$ on Bank, and $0.9506$ on Spambase). Adding base-model epistemic uncertainty (Model~B) produces modest improvements. Adding Venn--Abers width $w$ (Model~C) provides an additional statistically detectable contribution on all three datasets: $\Delta R^2 = +0.0208$ on Adult (95\% bootstrap CI $[0.0095, 0.0377]$), $\Delta R^2 = +0.0171$ on Bank (95\% bootstrap CI $[0.0116, 0.0282]$), and $\Delta R^2 = +0.0045$ on Spambase (95\% bootstrap CI $[0.0015, 0.0090]$). All 95\% bootstrap confidence intervals for $\Delta R^2$ are strictly positive and bounded away from zero. These results are consistent with width containing calibration-support information not accounted for by outcome ambiguity or the base-model bootstrap measure.
