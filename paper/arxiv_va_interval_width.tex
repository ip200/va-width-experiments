...
\section{Problem Setup and Notation}
	...
	\begin{enumerate}
		\item \textbf{Idealised Sample-Size Scaling (Figure~\ref{fig:idealised_scaling}):} To test the pure sample-size scaling exponents ($-2/3$ for interval width, $-1/3$ for pointwise standard deviation and $U_{\mathrm{cal}}$), we generate scores $S \sim U[0, 1]$ under a standard sigmoidal probability curve $\theta(s) = 1/(1 + \exp(-4(s - 0.5)))$. We evaluate the interval width $w_n(s_0)$ at a fixed interior test point $s_0 = 0.5$ across sample sizes $n \in [100, 20{,}000]$ using $R = 200$ independent Monte Carlo repetitions per sample size.
		\item \textbf{Multivariate Asymptotic Scaling ($W_2$ Factorial Design, Table~\ref{tab:exponent_convergence} and Equation~\ref{eq:pooled_w2}):} To verify the joint dependency on score density $f(s)$, Bernoulli variance $v(s) = \theta(s)(1 - \theta(s))$, and curve derivative $\theta'(s)$ in Equation~\ref{eq:scaling_law_final}, we conduct a comprehensive simulation across four score density families (uniform, Gaussian, symmetric Beta, and asymmetric Beta), six probability curves $\theta(s)$ (linear, moderate sigmoid, sharp sigmoid, quadratic, cubic, and sinusoidal), and 14 interior evaluation scores $s \in [-0.65, 0.65]$ across sample sizes $n \in [500, 100{,}000]$.
		\item \textbf{Non-Monotonic Geometric Regimes (Figure~\ref{fig:non_monotonic_scaling_laws}):} To evaluate the asymptotic impact of monotonicity violations and zero derivatives derived in Section~\ref{sec:theory}, we evaluate scores $S \sim U[-1, 1]$ under the four geometric regimes (monotonic, violating, interior extremum, and flat boundary extremum) across sample sizes $n \in [100, 5{,}000]$.
	\end{enumerate}
...
\end{figure}
	...
	Across all three datasets, Model~A (score-conditional outcome ambiguity alone) accounts for a large baseline fraction of calibration variability ($R^2$ of $0.8823$ on Adult, $0.9149$ on Bank, and $0.9506$ on Spambase). Adding base-model epistemic uncertainty (Model~B) produces modest improvements. Adding Venn--Abers width $w$ (Model~C) provides additional explanatory power across all three datasets: $\Delta R^2 = +\AdultDeltaRsq{}$ on Adult (95\% bootstrap CI $[\AdultDeltaRsqCILow{}, \AdultDeltaRsqCIHigh{}]$), $\Delta R^2 = +\BankDeltaRsq{}$ on Bank (95\% bootstrap CI $[\BankDeltaRsqCILow{}, \BankDeltaRsqCIHigh{}]$), and $\Delta R^2 = +\SpambaseDeltaRsq{}$ on Spambase (95\% bootstrap CI $[\SpambaseDeltaRsqCILow{}, \SpambaseDeltaRsqCIHigh{}]$). The bootstrap intervals exclude zero, although the lower endpoint for Spambase is very close to zero ($1.65 \times 10^{-5}$). These results are consistent with width containing calibration-support information not accounted for by outcome ambiguity or the base-model bootstrap measure.
