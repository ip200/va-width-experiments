...
\section{Problem Setup and Notation}
	...
	The Venn--Abers predictor outputs the interval $[p_0(s), p_1(s)]$. By construction, $p_0(s) \le p_1(s)$. As introduced in Section~\ref{sec:intro}, for empirical analyses we summarize the pair by the midpoint $\hat{p}(s)$, while the interval width is $w_n(s)$:
	\begin{equation}
	...
	\input{table_calibration_resampling.tex}
	
	Table~\ref{tab:calibration_resampling} shows that both quantities decrease as the calibration set becomes larger. The correlation between width and resampling SD is high across all calibration sizes in this experiment, ranging from about 0.92 to 0.97.
...
\section{Conclusion}
	...
	\end{equation}
	This cancellation shows that the scaling law is mathematically invariant to strictly increasing score transformations (even if the transformation derivative $g'(s)$ varies non-linearly), which is consistent with the rank-ordering nature of the PAV algorithm.
