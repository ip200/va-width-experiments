...
\section{Conclusion}
	...
	Let $h: \mathbb{R} \to \mathbb{R}$ be a strictly increasing, differentiable function defining a score transformation $s_{\text{new}} = h(s)$ (distinct from the base scoring function $g$ of Section~\ref{sec:setup}, which is held fixed throughout this appendix). Let $\theta(s)$ and $f(s)$ denote the true probability function and score density under the original score space, respectively. Under the transformed score space:
	\begin{enumerate}
		...
		\begin{equation}
			\theta'_{\text{new}}(s_{\text{new}}) = \frac{d\theta_{\text{new}}}{d s_{\text{new}}} = \frac{d\theta}{ds} \frac{ds}{ds_{\text{new}}} = \theta'(s) \cdot \frac{1}{h'(s)}
		\end{equation}
		\item The probability density of the scores transforms under change-of-variables as:
		\begin{equation}
			f_{\text{new}}(s_{\text{new}}) = f(s) \cdot \left| \frac{ds}{ds_{\text{new}}} \right| = f(s) \cdot \frac{1}{h'(s)}
		\end{equation}
	\end{enumerate}
	...
	\begin{align}
		...
		&= \left( \frac{f(s)}{h'(s)} \right)^{-2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot \left( \frac{\theta'(s)}{h'(s)} \right)^{2/3} \\
		&= f(s)^{-2/3} \cdot (h'(s))^{2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot (\theta'(s))^{2/3} \cdot (h'(s))^{-2/3}
	\end{align}
	Notice that the derivative factors $(h'(s))^{2/3}$ and $(h'(s))^{-2/3}$ cancel each other exactly:
	\begin{equation}
		w(s_{\text{new}}) \propto f(s)^{-2/3} \cdot (\theta(s)(1-\theta(s)))^{-1/3} \cdot (\theta'(s))^{2/3} = w(s)
	\end{equation}
	This cancellation shows that the scaling law is mathematically invariant to strictly increasing score transformations (even if the transformation derivative $h'(s)$ varies non-linearly), which is consistent with the rank-ordering nature of the PAV algorithm.
